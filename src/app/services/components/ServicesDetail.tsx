      ...
      <div className="container max-w-7xl mx-auto px-4 lg:px-8">
        <div className="flex flex-col gap-16">
          {content.servicesList?.map((service: any, i: number) => (
            <div
              key={i}
              id={service.id || `service-${i}`}
              className={`fade-up grid grid-cols-1 lg:grid-cols-2 gap-10 items-start pb-16 ${i < content.servicesList.length - 1 ? 'border-b border-border' : ''}`}
            >
              {/* Left */}
              <div className="flex flex-col gap-5">
                ...
                <Link href={service.href || '/for-employers'} className="btn-primary self-start">
