...
export default function TrustStrip() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeDocument('site_content', 'homepage', (doc) => {
      if (doc) {
        setContent(prev => ({
          trustList: doc.trustList || prev.trustList,
        }));
      }
    });
    return () => unsubscribe();
    ...
    <section ref={ref} className="py-16 bg-background" aria-label="Trust values">
      <div className="container max-w-7xl mx-auto px-4 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {content.trustList?.map((item: any, i: number) => (
            <div
              key={i}
            ...
            >
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">{item.desc || item.text}</p>
            </div>
          ))}
        </div>
      </div>
